...
import {payloadName, entityName, entityNameArr} from "../constant/config";
...
let handleMessage = async (sender_psid, message) => {
    ...
    if (message && message.quick_reply && message.quick_reply.payload) {
        if (message.quick_reply.payload === payloadName.SMALL
            || message.quick_reply.payload === payloadName.MEDIUM
            || message.quick_reply.payload === payloadName.LARGE) {
            //asking about phone number
            if (message.quick_reply.payload === payloadName.SMALL ) user.quantity = ChatBotLabel(lang).title.quantity1;
            if (message.quick_reply.payload === payloadName.MEDIUM) user.quantity = ChatBotLabel(lang).title.quantity2;
            if (message.quick_reply.payload === payloadName.LARGE) user.quantity = ChatBotLabel(lang).title.quantity3;
    ...
    let locale = entity.locale || 'en';

    if (entity.name === entityName.datetime) {
    ...
    } else if (entity.name === entityName.phone) {
...
let handlePostback = async (sender_psid, received_postback) => {
    ...
    switch (payload) {
        case payloadName.GET_STARTED:
        case payloadName.RESTART_CONVERSATION:
